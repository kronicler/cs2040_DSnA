...
// The creation of two seperate trees
BSTVertex *root_male = NULL, *root_female = NULL;
...
BSTVertex* remove(BSTVertex* T, string v) {
    ...
    if (T->key == v) {                         // this is the node to be deleted
        if (T->left == NULL && T->right == NULL)                 // this is a leaf
            ...
            string successorV = successor(T)->key;
...
int main () {
    ...
    while (true) {
        ...
        if (!command) break;
            ...
            cin >> name >> gender;
            if (gender == 1) {
                root_male =  insert(root_male, name);
            }
            else {
                root_female = insert(root_female, name);
            }
        }
            ...
            if (gender == 0) {
                ...
                BSTVertex * it_ = search_lower_bound(root_male, start);
                BSTVertex  * it2_ = search_lower_bound(root_male, end);
                ...
                it_ = search_lower_bound(root_female, start);
                it2_ = search_lower_bound(root_female, end);
                
                while (it_ != NULL) {
                    if (it_ == it2_) break;
                    count++;
                    it_ = successor(it_); // Doesnt matter female or male they wont cross paths
                }
            }
            else if (gender == 1) {
                BSTVertex * it_ = search_lower_bound(root_male, start);
                ...
                BSTVertex * it_ = search_lower_bound(root_female, start);
                BSTVertex  * it2_ = search_lower_bound(root_female, end);
